...
export const ArticleImageBlock: FC<IArticleImageBlockProps> = memo(
    function ArticleImageBlock(props) {
        const { className, block } = props

        return (
            <div className={cn(styles.wrapper, className)}>
                <img className={styles.img} src={block.src} alt={block.title} />
                {block.title && <span className={styles.title}>{block.title}</span>}
            </div>
        )
    },
)
